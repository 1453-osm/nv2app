...
class NotificationSchedulerService {
  ...
  Future<void> rescheduleTodayNotifications() async {
    ...
    await _scheduleFor('imsak', fajrStr, 0);
    await _scheduleFor('gunes', sunriseStr, 0);
    await _scheduleFor('ogle', dhuhrStr, 0);
    await _scheduleFor('ikindi', asrStr, 0);
    await _scheduleFor('aksam', maghribStr, 0);
    await _scheduleFor('yatsi', ishaStr, 0);
    ...
    if (tomorrowFajr != null && tomorrowFajr.isNotEmpty) {
      await _scheduleFor('imsak', tomorrowFajr, 1);
    }
    if (tomorrowSunrise != null && tomorrowSunrise.isNotEmpty) {
      await _scheduleFor('gunes', tomorrowSunrise, 1);
    }
    if (tomorrowDhuhr != null && tomorrowDhuhr.isNotEmpty) {
      await _scheduleFor('ogle', tomorrowDhuhr, 1);
    ...
    if (tomorrowAsr != null && tomorrowAsr.isNotEmpty) {
      await _scheduleFor('ikindi', tomorrowAsr, 1);
    }
    if (tomorrowMaghrib != null && tomorrowMaghrib.isNotEmpty) {
      await _scheduleFor('aksam', tomorrowMaghrib, 1);
    }
    if (tomorrowIsha != null && tomorrowIsha.isNotEmpty) {
      await _scheduleFor('yatsi', tomorrowIsha, 1);
    }
  ...
  Future<void> _scheduleSpecialFridayNotification(String dhuhrTime, int dayOffset) async {
    ...
    final bool enabled = prefs.getBool('${base}enabled') ?? true;
    final int minutes = prefs.getInt('${base}minutes') ?? 30;
    final String soundId = prefs.getString('${base}sound') ?? 'default';
    
    if (kDebugMode) {
      // ignore: avoid_print
      print('NotificationScheduler: Schedule Friday prayer at $dhuhrTime (day+$dayOffset), enabled=$enabled, minutes=$minutes, sound=$soundId');
    }
  ...
  String _bodyFor(String id, int minutes) {
    if (minutes == 0) {
      return _getImmediateMessage(id);
    } else {
      return _getAdvanceMessage(id, minutes);
    }
  }
